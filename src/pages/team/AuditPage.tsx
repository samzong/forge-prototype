...
import type { App, AuditAction, AuditEvent, User } from '@/types'
...
export default function AuditPage() {
  ...
  const userMap = useMemo(() => {
    ...
    for (const u of members?.items ?? []) m.set(u.id, u)
    return m
  }, [members])
    ...
    <div className="space-y-6">
      <section className="bg-card border border-line rounded-xl p-6">
        <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
          <div>
            <h2 className="text-[14px] font-extrabold text-fg inline-flex items-center gap-[6px]">
              <Users size={14} className="text-fg-muted" />
              Team activity
            </h2>
            <p className="text-[12.5px] text-fg-muted mt-[3px] max-w-[560px]">
              Every lifecycle action scoped to <span className="font-semibold text-fg">{team.name}</span> —
              deploys, rollbacks, shares, forks. Retained for 180 days.
            </p>
          </div>
      ...
      {loading && events.length === 0 ? (
        ...
        <section className="bg-card border border-line rounded-xl p-6">
          <EmptyState
            ...
            hint={
              filter === 'all'
                ? 'Lifecycle actions across team-owned apps will appear here.'
        ...
        <section className="bg-card border border-line rounded-xl p-6">
          <Timeline events={events} appMap={appMap} userMap={userMap} />
        </section>
      )}
    </div>
  )
...
function TimelineRow({
  ...
  const actorLabel = actor?.displayName ?? event.actorId
  return (
    <motion.div
    ...
    >
      ...
      <div className="bg-bg border border-line rounded-[10px] p-4">
        ...
        <div className="mt-2 text-[12px] text-fg-muted flex items-center gap-[6px] flex-wrap">
          <span>by</span>
          <span className="font-mono text-fg font-semibold">{actorLabel}</span>
          {actor && (
            <span className="font-mono text-[10.5px] text-fg-subtle">· {actor.email}</span>
          )}
        </div>
        {event.note && (
